...
/**
 * 类描述:〈校验MQ消息内容，趸交计划的MQ〉
 *
 * @author: jinjianxu
 * @since: 1.0
 */
@MqVerifyService(scheduleType = ScheduleTypeEnum.PREMIUM)
public class PremiumMqVerifyProcessor extends AbstractMqVerifyProcessor {
  ...
  public boolean doVerify(String payNo, String id) {
    ...
    if (null == singlePremiumScheduleEntity) {
      logger.error("没有找到id=[{}]的qs_single_premium_schedule记录", id);
      return false;
    }
    userId = String.valueOf(singlePremiumScheduleEntity.getUserId());
  ...
  public SinglePremiumScheduleDao getSinglePremiumScheduleDao() {
    return singlePremiumScheduleDao;
  }


  public void setSinglePremiumScheduleDao(
      SinglePremiumScheduleDao singlePremiumScheduleDao) {
    this.singlePremiumScheduleDao = singlePremiumScheduleDao;
  }
