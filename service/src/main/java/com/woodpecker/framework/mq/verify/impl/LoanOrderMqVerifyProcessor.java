...
/**
 * 类描述:〈校验MQ消息内容，loanorder的MQ〉
 *
 * @author: jinjianxu
 * @since: 1.0
 */
@MqVerifyService(scheduleType = ScheduleTypeEnum.LOAN_ORDER)
public class LoanOrderMqVerifyProcessor extends AbstractMqVerifyProcessor {
    ...
    for (JSONObject json : jsonObjectList) {
      ...
      if (array.size() > 0) {
        ...
        if (flag) {
          ...
          String loanOrderIdList = messageBody.getString(Constants.LOANORDERIDLIST);
          //判断这条MQ消息是否是我们找的MQ
          if (StringUtil.equals(actPayNo, payNo) && StringUtil.equals(actUserId, userId)
              && StringUtil.contains(loanOrderIdList, id)) {
