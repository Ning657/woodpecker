...
public class BindCheckPointServiceImpl implements BindCheckPointService {
  ...
  public void checkBankAccountTable(BankAccountEntity bankAccountEntity,
    ...
    for (BindCardEnum bindCardEnum : bindCardEnums) {
      ...
      boolean isExistBindId = JsonUtil.parseObject(bindId)
          .containsKey(String.valueOf(bindCardEnum.getChannel()));
      Validate.isTrue(isExistBindId,
          "校验t_bank_account表中的BindId字段包含[" + bindCardEnum.getChannel() + "]");
      //检查Channel字段
      String channel = "\"" + bankAccountEntity.getChannel() + "\"";
      boolean isExistChannel = channel.contains(String.valueOf(bindCardEnum.getChannel()));
      Validate.isTrue(isExistChannel,
          "校验t_bank_account表中的Channel字段包含[" + bindCardEnum.getChannel() + "]");
    }
    //检查Status字段
   ...
   */
  @Override
  public void checkAccountTable(AccountEntity accountEntity, BindCardEnum[] bindCardEnums) {
    ...
    for (BindCardEnum bindCardEnum : bindCardEnums) {
      ...
      boolean isExistChannel = channels.contains(String.valueOf(bindCardEnum.getChannel()));
      Validate.isTrue(isExistChannel,
          "校验t_tp_account表中的channel字段包含[" + bindCardEnum.getChannel() + "]");
    }
  }
