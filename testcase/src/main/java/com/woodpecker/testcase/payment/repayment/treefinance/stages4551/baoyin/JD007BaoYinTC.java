...
/**
 * 类描述:〈包银走京东两方代扣(007)，通道42〉
 *
 * @author: jinjianxu
 * @since: 1.0
 */
public class JD007BaoYinTC extends Stages4551TestCase {

  /**
   * 用例的支付渠道，京东007 - 42（不可改）
   */
  protected final int channel = 42;

  /**
   * 资金渠道，指定为包银渠道，不可改
   */
  protected final PlatformIdEnum platformIdEnum = PlatformIdEnum.BY;

  protected String version = "1";

  protected String orderId;

  protected String loanOrderId;
  ...
  public void collectionWithhold() {
    ...
    //读取还款计划表的前面二期的还款计划
    //因为第一期是正常还款，走的是「中信内部户7901」，也就是「京东两方代扣（009）44」
    //第一期
    ...
    //对第一期进行还款
    //通过催收代扣方式还款
    HttpResponse httpResponse = repaymentFactory
        .collectionWithhold(Integer.valueOf(userId), (long) firstRepaymentSchedule.getId());
    String result1 = httpResponse.getContent();
