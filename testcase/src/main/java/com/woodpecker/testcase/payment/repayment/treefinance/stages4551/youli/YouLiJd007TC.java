...
/**
 * 类描述:〈有利走京东两方代扣007(JDAGREEMENTPAY_4551)，通道42〉
 *
 * @author: jinjianxu
 * @since: 1.0
 */
public class YouLiJd007TC extends Stages4551TestCase {
  ...
  public void collectionWithhold() {
    ...
    //将趸交计划置为已结清
    super.orderService.clearSinglePremiumSchedule(Integer.parseInt(loanOrderId));
    //对第一期进行还款
    //第一期
    RepaymentScheduleEntity repaymentSchedule = super.repaymentScheduleService
        .getRepaymentSchedule(Integer.parseInt(loanOrderId), (byte) 1);
    long id = Long.parseLong(loanOrderId);
    int scheduleId = repaymentSchedule.getId();
    BigDecimal amount = repaymentSchedule.getAmount();
    //还款
    HttpResponse httpResponse = super.repayment
        .jdRepaymentByCollectionDeposit(id, amount, super.routerEnvVersion);
    //校验是否请求成功
    ...
    super.repaymentCheckPointService
        .checkMqSendSuccess(payNo, loanOrderId, ScheduleTypeEnum.LOAN_ORDER);
